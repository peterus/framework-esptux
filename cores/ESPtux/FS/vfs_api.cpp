...
FileImplPtr VFSImpl::open(const char *path, const char *mode) {
  if (!_mountpoint) {
    log_e(SysFS, "File system is not mounted");
    return FileImplPtr();
  }

  if (!path || path[0] != '/') {
    log_e(SysFS, "%s does not start with /", path);
    return FileImplPtr();
  }

  char *temp = (char *)malloc(strlen(path) + strlen(_mountpoint) + 2);
  if (!temp) {
    log_e(SysFS, "malloc failed");
    return FileImplPtr();
  }
  ...
  if (!stat(temp, &st)) {
    ...
    log_e(SysFS, "%s has wrong mode 0x%08X", path, st.st_mode);
    return FileImplPtr();
  }
  ...
  log_e(SysFS, "%s does not exist", temp);
  free(temp);
  return FileImplPtr();
}
...
bool VFSImpl::rename(const char *pathFrom, const char *pathTo) {
  ...
  if (!pathFrom || pathFrom[0] != '/' || !pathTo || pathTo[0] != '/') {
    log_e(SysFS, "bad arguments");
    return false;
  }
  if (!exists(pathFrom)) {
    log_e(SysFS, "%s does not exists", pathFrom);
    return false;
  }
...
bool VFSImpl::remove(const char *path) {
  ...
  if (!f || f.isDirectory()) {
    ...
    log_e(SysFS, "%s does not exists or is directory", path);
    return false;
  }
  f.close();
...
bool VFSImpl::rmdir(const char *path) {
  ...
  if (!f || !f.isDirectory()) {
    ...
    log_e(SysFS, "%s does not exists or is a file", path);
    return false;
  }
  f.close();
...
VFSFileImpl::VFSFileImpl(VFSImpl *fs, const char *path, const char *mode) : _fs(fs), _f(NULL), _d(NULL), _path(NULL), _isDirectory(false), _written(false) {
  ...
  _path = strdup(path);
  if (!_path) {
    log_e(SysFS, "strdup(%s) failed", path);
    free(temp);
    return;
  }
  ...
  if (!stat(temp, &_stat)) {
    ...
    if (S_ISREG(_stat.st_mode)) {
      ...
      _f           = fopen(temp, mode);
      if (!_f) {
        log_e(SysFS, "fopen(%s) failed", temp);
      }
    } else if (S_ISDIR(_stat.st_mode)) {
      _isDirectory = true;
      _d           = opendir(temp);
      if (!_d) {
        log_e(SysFS, "opendir(%s) failed", temp);
      }
    } else {
      log_e(SysFS, "Unknown type 0x%08X for file %s", ((_stat.st_mode) & S_IFMT), temp);
    }
  } else {
    // file not found
    if (!mode || mode[0] == 'r') {
